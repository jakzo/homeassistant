...
from requests import Response, post, request  # http request library
...
from .device import Device, Softener, Vacuum

# Custom error handling
from .exc import AylaAuthError, AylaAuthExpiringError, AylaNotAuthedError
...
class AylaApi:
    ...
    def sign_in(self):
        ...
        resp = post(
            f"{self.eu_user_field_url if self.europe else self.user_field_url:s}/users/sign_in.json",
            json=login_data,
        )
        self._set_credentials(resp.status_code, resp.json())
    ...
    def refresh_auth(self):
        ...
        resp = post(
            f"{self.eu_user_field_url if self.europe else self.user_field_url:s}/users/refresh_token.json",
            json=refresh_data,
        )
        self._set_credentials(resp.status_code, resp.json())
    ...
    def sign_out(self):
        ...
        post(
            f"{self.eu_user_field_url if self.europe else self.user_field_url:s}/users/sign_out.json",
            json=self._sign_out_data,
        )
        self._clear_auth()
    ...
    @property
    def auth_expiration(self) -> datetime | None:
    ...
    def check_auth(self, raise_expiring_soon=True):
        ...
        if raise_expiring_soon and self.token_expiring_soon:
    ...
    def get_user_profile(self) -> dict[str, str]:
        ...
        resp = self.self_request(
            "get",
            f"{self.eu_user_field_url if self.europe else self.user_field_url:s}/users/get_user_profile.json",
        )
        response = resp.json()
    ...
    def list_devices(self) -> list[dict]:
        ...
        resp = self.self_request(
            "get",
            f"{self.eu_ads_url if self.europe else self.ads_url:s}/apiv1/devices.json",
        )
        devices = resp.json()
        if resp.status_code == 401:
    ...
    def get_devices(self, update: bool = True) -> list[Device]:
        ...
        for d in self.list_devices():
            if d["product_name"] in self._vacuum_devices:
                ...
                d["product_name"] in self._softener_devices
                or "Softener" in d["product_name"]
            ):
    ...
    def get_actions(self) -> dict[str, str]:
        ...
        resp = self.self_request(
            "get",
            f"{self.eu_rulesservice_url if self.europe else self.rulesservice_url:s}/rulesservice/v1/actions.json",
        )
    ...
    def get_rules(self) -> dict[str, str]:
        ...
        resp = self.self_request(
            "get",
            f"{self.eu_rulesservice_url if self.europe else self.rulesservice_url:s}/rulesservice/v1/rules.json",
        )
    ...
    def get_rule_action(self, rule_uuid) -> dict[str, str]:
        ...
        resp = self.self_request(
            "get",
            f"{self.eu_rulesservice_url if self.europe else self.rulesservice_url:s}/rulesservice/v1/rules/{rule_uuid}/actions.json",
        )
        response = resp.json()["actions"]
        if resp.status_code == 401:
            raise AylaAuthError(response)
    ...
    def get_commands(self, device_id) -> dict[str, str]:
        ...
        resp = self.self_request(
            "get",
            f"{self.eu_ads_url if self.europe else self.ads_url:s}/apiv1/devices/{device_id}/commands.json",
        )
        response = resp.json()
        if resp.status_code == 401:
            raise AylaAuthError(response)
    ...
    def get_all_notifications(self, device_id) -> dict[str, str]:
        ...
        resp = self.self_request(
            "get",
            f"{self.eu_ads_url if self.europe else self.ads_url:s}/apiv1/devices/{device_id}/notifications/all.json",
        )
