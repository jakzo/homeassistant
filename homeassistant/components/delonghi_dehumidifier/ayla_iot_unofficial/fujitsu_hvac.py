...
from .device import AylaReadOnlyPropertyError, Device, PropertyName, PropertyValue
...
class FujitsuHVAC(Device):
    ...
    async def async_set_property_value(
        ...
        async with await self.ayla_api.async_request(
            "post", end_point, json=data, headers={"x-ayla-source": "Mobile"}
        ) as resp:
    ...
    async def async_get_last_datapoint(self, property: str) -> dict[str, Any] | None:
        ...
        extra_param = {"limit": 1}
        async with await self.ayla_api.async_request(
            "get", endpoint, params=extra_param
        ) as resp:
    ...
    def poll_while(self, property: str, keep_polling_value: Any):
        ...
        while count < 10:
            ...
            if (
                datapoint
                and datapoint["value"] != keep_polling_value
                and datapoint["echo"] == True
            ):
                break
    ...
    def op_mode(self, val: OpMode):
        if val not in self.supported_op_modes:
            raise SettingNotSupportedError(
                f"Device does not support operation mode {val.name}"
            )
    ...
    def fan_speed(self, val: FanSpeed):
        if val not in self.supported_fan_speeds:
            raise SettingNotSupportedError(
                f"Device does not support fan speed {val.name}"
            )
    ...
    def outdoor_temperature(self) -> float | None:
        if (
            OUTDOOR_TEMPERATURE in self.property_values
    ...
    def temperature_range_for_mode(self, mode: OpMode) -> (float, float):
        if mode not in self.supported_op_modes:
            raise SettingNotSupportedError(
                f"Device does not support operation mode {mode.name}"
            )
    ...
    def set_temp(self, val: float):
        ...
        self.set_property_value(
            ADJUST_TEMPERATURE, int((round(val * 2.0) / 2.0) * 10.0)
        )

    async def async_set_set_temp(self, val: float):
        await self.async_set_property_value(
            ADJUST_TEMPERATURE, int((round(val * 2.0) / 2.0) * 10.0)
        )
    ...
    def swing_mode(self, val: SwingMode):
        if val not in self.supported_swing_modes:
            raise SettingNotSupportedError(
                f"Device does not support swing mode {val.name}"
            )
    ...
    def horizontal_swing(self) -> bool:
        ...
        return (
            self.property_values[HORIZ_SWING_PARAM_MAP[self.model]]
            == SWING_VAL_MAP[self.model][True]
        )
    ...
    def horizontal_swing(self, val: bool):
        ...
        self.set_property_value(
            HORIZ_SWING_PARAM_MAP[self.model], SWING_VAL_MAP[self.model][val]
        )
    ...
    def vertical_swing(self) -> bool:
        ...
        return (
            self.property_values[VERT_SWING_PARAM_MAP[self.model]]
            == SWING_VAL_MAP[self.model][True]
        )
    ...
    def vertical_swing(self, val: bool):
        ...
        self.set_property_value(
            VERT_SWING_PARAM_MAP[self.model], SWING_VAL_MAP[self.model][val]
        )
