...
class Device:
    ...
    def __init__(self, ayla_api: "AylaApi", device_dct: dict, europe: bool = False):
        ...
        self.properties_full = defaultdict(
            dict
        )  # Using a defaultdict prevents errors before calling `update()`
    ...
    @property
    def device_model_number(self) -> str | None:
    ...
    async def async_get_metadata(self):
        """Fetch device metadata.  Not needed for basic operation."""
        async with await self.ayla_api.async_request(
    ...
    async def async_set_property_value(
        self, property_name: PropertyName, value: PropertyValue
    ):
    ...
    def update(self, property_list: Iterable[str] | None = None):
        ...
        resp = self.ayla_api.self_request(
            "get", self.dsn_all_properties_endpoint, params=params
        )
    ...
    @staticmethod
    def _get_most_recent_datum(
        data_list: list[dict], date_field: str = "updated_at"
    ) -> dict:
        """Get the most recent data point from a list of annoyingly nested values"""
        datapoints = {
            _parse_datetime(d["datapoint"][date_field]): d["datapoint"]
            for d in data_list
            if "datapoint" in d
        }
        if not datapoints:
    ...
    def get_file_property_url(self, property_name: PropertyName) -> str | None:
    ...
    async def async_get_file_property_url(
        self, property_name: PropertyName
    ) -> str | None:
        """File properties are versioned and need a special lookup"""
        try:
            url = self._get_file_property_endpoint(property_name)
    ...
    def get_property_triggers(self, property_name: PropertyName):
        ...
        resp = self.ayla_api.self_request(
            "get", self.property_triggers_endpoint(property_name)
        )
        triggers = resp.json()
        # [{trigger:{key}}]
    ...
    async def async_get_property_triggers(self, property_name: PropertyName):
        """For a properties triggers, get them and add them to the device"""
        resp = await self.ayla_api.async_request(
            "get", self.property_triggers_endpoint(property_name)
        )
...
class Softener(Device):
    ...
    def __init__(self, ayla_api: "AylaApi", device_dct: dict, europe: bool = False):
        ...
        self.hourly_usage_properties = [
            ...
            "hourly_usage_hour_8",
            "hourly_usage_hour_9",
            "hourly_usage_hour_10",
            "hourly_usage_hour_11",
            "hourly_usage_hour_12",
            "hourly_usage_hour_13",
            "hourly_usage_hour_14",
            "hourly_usage_hour_15",
            "hourly_usage_hour_16",
            "hourly_usage_hour_17",
            "hourly_usage_hour_18",
            "hourly_usage_hour_19",
            "hourly_usage_hour_20",
            "hourly_usage_hour_21",
            "hourly_usage_hour_22",
            "hourly_usage_hour_23",
            "hourly_usage_hour_24",
        ]
    ...
    def set_property_value(self, property_name: PropertyName, value: PropertyValue):
        ...
        resp = self.ayla_api.self_request(
            "post",
            self.datapoints_endpoint,
            json=self.set_datapoint_payload(property_name, value),
        )
    ...
    def send_poll(self):
        ...
        resp = self.ayla_api.self_request(
            "post",
            self.datapoints_endpoint,
            json=self.set_datapoint_payload("wifi_report", 1),
        )
        if resp.json():
            return True
        return False
    ...
    async def async_send_poll(self):
        ...
        async with await self.ayla_api.async_request(
            "post", datapoints, json=self.set_datapoint_payload("wifi_report", 1)
        ) as resp:
...
class DevicePropertiesView(abc.Mapping):
    ...
    def __getitem__(self, key):
        ...
        try:
            ...
            _LOGGER.warning(
                "Error converting property type (value: %r, type: %r)",
                value,
                value_type,
                exc_info=exc,
            )
