...
class Session:
    ...
    def update(self, sessionInfo: SessionInfo, privateKey: ec.EllipticCurvePrivateKey):
        ...
        if self.publicKey != sessionInfo.publicKey:
            ...
            self.hmac = hmac.new(key, b"authenticated command", hashlib.sha256).digest()
    ...
    async def _sign(
        ...
        if resp.HasField("protobuf_message_as_bytes"):
            if resp.from_destination.domain == DOMAIN_VEHICLE_SECURITY:
                ...
                if vcsec.commandStatus.operationStatus == OPERATIONSTATUS_OK:
                    return {"response": {"result": True, "reason": ""}}
                if vcsec.commandStatus.operationStatus == OPERATIONSTATUS_WAIT:
                    attempt += 1
                    if attempt > 3:
                        # We tried 3 times, give up, raise the error
                        return {
                    ...
                    async with session.lock:
                        await sleep(2)
                    return await self._sign(domain, command, attempt)
                if vcsec.commandStatus.operationStatus == OPERATIONSTATUS_ERROR:
