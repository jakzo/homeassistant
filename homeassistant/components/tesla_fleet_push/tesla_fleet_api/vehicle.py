...
class Vehicle:
    ...
    def pre2021(self, vin: str) -> bool:
        ...
        return vin[3] in ["S", "X"] and (
            vin[9] <= "L" or (vin[9] == "M" and vin[7] in ["1", "2", "3", "4"])
        )
    ...
    async def fleet_status(self, vins: builtins.list[str]) -> dict[str, Any]:
    ...
    async def remove_charge_schedule(
        ...
        return await self._request(
            Method.POST,
            f"api/1/vehicles/{vehicle_tag}/command/remove_charge_schedule",
            json={"id": id},
        )
    ...
    async def remove_precondition_schedule(
        ...
        return await self._request(
            Method.POST,
            f"api/1/vehicles/{vehicle_tag}/command/remove_precondition_schedule",
            json={"id": id},
        )
